	...
	try {
		...
		if (!await video.isMuxed() && !await video.isDownloaded()) {
			...
			downloadRequest.on("downloadProgress", downloadProgress => {
				...
				downloadStats[formattedTitle] = { totalMB, downloadedMB, downloadSpeed, timeElapsed: 0 };
				updateSummaryBar();
			});
		...
		mpb.updateTask(formattedTitle, { message: `\u001b[31m\u001b[1mERR\u001b[0m: ${error.message}` });

		// Retry downloading the video if this is the first failure.
		if (retries === 0) {
			mpb.updateTask(formattedTitle, { message: `\u001b[31m\u001b[1mERR\u001b[0m: ${error.message} - Retrying...` });
			await sleep(1000);
		...
		if (error.message.includes("The server aborted pending request") && retries < 3) {
			mpb.updateTask(formattedTitle, { message: `\u001b[31m\u001b[1mERR\u001b[0m: ${error.message} - Retrying ${retries}/3` });
			await sleep(1000);
			await processVideo(video, ++retries);
			return;
		}
		...
		if (error.message.includes("Response code 400") || error.message.includes("Response code 404")) {
			...
			if (currentResIndex !== 0) {
				...
				mpb.updateTask(formattedTitle, { message: `\u001b[31m\u001b[1mERR\u001b[0m: ${error.message} - Retrying at ${newRes}p` });
				await sleep(1000);
