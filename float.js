...
function checkExistingVideos () {
  Object.keys(videos).forEach(function (key) {
    ...
    let file = fs.statSync(videos[key].file);
    if (videos[key].saved === true || file.size < 10000) {
      delete videos[key];
    }
  });
}
  ...
  return new Promise((resolve, reject) => {
    ...
    Object.keys(settings.subscriptions).forEach(function (key) {
      ...
      for (i = 1; i <= Math.ceil(settings.maxVideos / 20); i++) {
        let vUrl = `https://www.floatplane.com/api/creator/videos?creatorGUID=${subscription.id}&fetchAfter=${((i * 20) - 20)}`;
...
function doTitleFormatting (video) {
  ...
  */
  ...
  video.episodeTitle = sanitize(video.title);
...
function doPathChecks (video) {
  ...
  */
  ...
  if (settings.monthsAsSeasons) { // If your formatting the videos with the YEAR+MONTH as the season then
    let date = new Date(video.releaseDate); // Generate a new date from the publish date pulled above
    if (date.getMonth() < 10) { // If the month is less than 10 add a 0 to it
      seasonNumber = date.getFullYear() + '0' + date.getMonth(); // Set the seasonNumber to be the YEAR+MONTH, eg 201801
      rawPath = rawPath + seasonNumber + '/'; // Set the raw path to include the new season folder
    } else {
      seasonNumber = date.getFullYear() + date.getMonth();
    ...
    seasonNumber = date.getFullYear(); // Set the seasonNumber to be the YEAR, eg 2018
...
function downloadYoutube (video) {
  ...
  ytdl(video.url, {quality: 'highest'}).on('progress', function (length, bytesDownloaded, totalBytes) { // Send the request to download the file, run the below code every downloadUpdateTime while downloading
    ...
    let file = video.rawPath + video.title + '.mp4'; // Specifies where the video is saved
    let name = video.title.replace(/^.*[0-9].- /, '').replace('- ', ''); // Generate the name used for the title in metadata (This is for plex so "episodes" have actual names over Episode1...)
...
function downloadVideo (video) { // This handles resuming downloads, its very similar to the download function with some changes
  ...
  if (videos[video.guid].partial && !partial_data[video.guid].failed) { // If this video was partially downloaded
    ...
    let displayTitle = pad(`${colourList[video.subChannel]}${video.subChannel}\u001b[0m${video.title.replace(/.*- /, '> ').slice(0, 35)}`, 36); // Set the title for being displayed and limit it to 25 characters
