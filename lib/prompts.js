...
const resolutions = require('./floatplane/resolutions.json');
...
/**
 * Prompts user if they want to find the best download server now.
 * @param {bool} initial Default value
 * @returns {bool} True or False
 */
const findBestServerNow = async (initial=true) => (await prompts({
	type: 'toggle',
	name: 'findBestServer',
...
/**
 * Proompts the user if they want to find the best download server automatically in the future.
 * @param {bool} initial Default value
 * @returns {bool} True or False
 */
const autoFindBestServer = async (initial=true) => (await prompts({
	type: 'toggle',
	name: 'bestEdge',
 ...
 */
const videoResolution = async (initial) => (await prompts({
	type: 'select',
	name: 'resolution',
	message: 'What resolution would you like to download in?',
	choices: resolutions.map(res => ({ title: res, value: res, disabled: false })),
	initial: resolutions.indexOf(initial)
})).resolution

console.log(resolutions.indexOf(1080))
 ...
 * @returns {{optionName: bool}} Extra settings
 */
const extras = async (options) => (await prompts({
	type: 'multiselect',
	name: 'extras',
	message: 'Enable/Disable Extra Options:',
	choices: Object.keys(options).map(option => ({ title: option, value: option, disabled: options[option] })),
