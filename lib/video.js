...
class Video {
	...
	/**
	 * @returns {string} Formatted filename of video.
	 */
	get file() {
		let folder = this.channel.folderPath
		let file = ''
		...
		if (settings.fileFormatting.monthsAsSeasons) { // If your formatting the videos with the YEAR+MONTH as the season then
			...
			seasonNumber = this.releaseDate.getFullYear()+date.getMonth()>9?'0'+date.getMonth():date.getMonth(); // Set the seasonNumber to be the YEAR+MONTH, eg 201801
			folder += `/S${seasonNumber}/` // Set the raw path to include the new season folder
		} else if (settings.fileFormatting.yearsAsSeasons) { // If your formatting the videos with the YEAR as the season then
			seasonNumber = this.releaseDate.getFullYear() // Set the seasonNumber to be the YEAR, eg 2018
			folder = `/S${seasonNumber}/`
		}
		
		if (settings.fileFormatting.formatWithChannel == true) file += `${this.channel.title} - ` // Add channel naming if requested
		if (settings.fileFormatting.formatWithEpisodes == true) file += `S${seasonNumber}E${this.episodeNumber} - ` // Add Episode Number
		if (settings.fileFormatting.formatWithDate == true) file += `${this.releaseDate} - ` // Add the upload date to the filename

		file += this.title.replace(/ - /g, ' ')

		return folder.replace(/\/\//, '/')+sanitize(file)
	}
}
